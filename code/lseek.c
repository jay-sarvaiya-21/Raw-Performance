...
void letsseek(fd1){
    ...
    long start, end;
    int check_flag=0;
    unsigned int count=0,x=1;
    gettimeofday(&timecheck, NULL);
    start = (long)timecheck.tv_sec * 1000 + (long)timecheck.tv_usec / 1000;
    while(lseek(fd1,1,SEEK_CUR)>0){
         ...
         if(++count==x)
         {
            ...
            if(res>5 && res<=15)
            {
                ...
                printf("Time taken for lseek is %.3f\n",res);
                //printf("Performance in MiB/s is =  %u\n" , ((block_size*block_count)/(unsigned int)(res*1024*1024)));
                printf("system calls per seconds is =  %f\n" , (count/res) );
                exit(0);
            }
        }
    }
    if(check_flag==0)
         {
            ...
            //printf("Performance in MiB/s is =  %u\n" , ((block_size*block_count)/(unsigned int)(res*1024*1024)));
            printf("PERFORMANCE in B/s is =  %f\n" , (count/res) );
        ...
        for(int j=0;j<block_size;j++)
         ...
         if(++block_count==x)
         {
            ...
            {   
                ...
                printf("Time taken is %.3f\n",res);
                printf("Performance in MiB/s is =  %u\n" , ((block_size*block_count)/(unsigned int)(res*1024*1024)));
                printf("Performance in B/s is =  %u\n" , ((block_size*block_count)/(unsigned int)(res)));
                break;
            }
         }   
        free(buffer);  }
...
int main(int argc,char *argv[]) {
    ...
    if(argc!=3 ){
        printf("Enter the correct parameteres to read/write \n");
        exit(0); }
     ...
     if (fd<0)
        {
            printf("Cannot open the file! enter the correct filename");
            exit(0);
        }
     ...
     close(fd1);
        ...
        printf("enter the right parameters");
    }  
      return 0;
}
